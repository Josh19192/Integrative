<?php
function manage_vaccine($database){ 
    // Get all records from the 'employee_tbl' table
    $vaccines_json = file_get_contents('http://localhost:4000/vaccines');
    $vaccines = json_decode($vaccines_json, true);
    ...
    if (!empty($vaccines)) {
        ...
        echo '<tr>';
        foreach ($vaccines[0] as $key => $value) {
        ...
        foreach ($vaccines as $vaccine) {
            ...
            echo '<a href="home.php?tab=update_vaccine&id=' . $vaccine['id'] . '" class="bx bx-edit-alt me-1"></a> &nbsp;&nbsp;';

            echo '<form style="display: inline;" id="deleteForm' . $vaccine['id'] . '" action="" method="POST">';
            echo '<input type="hidden" name="delete_vaccine" value="' . $vaccine['id'] . '">';
            echo '<button type="submit" style="border: none; color:maroon;" class="bx bx-trash me-1" onclick="return confirmDelete(' . $vaccine['id'] . ')"></button>';
            echo '</form>';
            echo '</td>';
            echo '</tr>';
        }
